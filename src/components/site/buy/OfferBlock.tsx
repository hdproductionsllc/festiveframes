...
// Client island. The conversion core, as a cart: choose a pack on the left,
// configure quantity + the optional letter set on the right, see a LIVE total,
// then one Checkout button. The server re-derives every amount, so this total
// is display-only and always matches.
export function OfferBlock() {
  ...
  const packTitle = bundleSelected ? copy.buy.offer.bundle.title : copy.buy.offer.single.title;
  ...
  // Bundle first — it's the recommended pick (one for each car, or a gift).
  const cards = [
    {
  ...
  const stepperBtn =
    "flex h-8 w-8 items-center justify-center rounded-md border border-brand-navy/30 text-lg text-brand-navy disabled:opacity-40";
    ...
    <section className="paper-grain" aria-labelledby="offer-heading">
      <div className="mx-auto max-w-5xl px-4 py-14 sm:px-6 sm:py-16">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-5 lg:items-start">
          ...
          <div className="space-y-4 lg:col-span-3">
            {cards.map((card) => (
              <button
                ...
                className={`relative flex w-full flex-col rounded-xl border-2 bg-brand-cream-soft p-5 text-left transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-navy ${
                  card.selected ? "border-brand-red shadow-md" : "border-brand-navy/15 hover:border-brand-navy/40"
                }`}
              >
                {card.popular && (
                  <span className="absolute -top-3 left-5 rounded-full bg-brand-red px-3 py-1 font-mkt-display text-xs font-bold uppercase tracking-widest text-brand-white">
                ...
                <ul className="mt-3 space-y-1.5 text-sm text-brand-ink/85">
                  {card.items.map((item) => (
                ...
                <span
                  className={`mt-4 inline-flex w-fit items-center justify-center rounded-md px-4 py-1.5 text-sm font-semibold uppercase tracking-wide ${
                    card.selected ? "bg-brand-red text-brand-white" : "border border-brand-navy text-brand-navy"
                  }`}
                >
          ...
          {/* Right: order summary */}
          <div className="rounded-xl border border-brand-navy/15 bg-brand-cream-soft p-5 lg:col-span-2 lg:sticky lg:top-24">
            <h3 className="font-mkt-display text-lg font-bold uppercase tracking-tight text-brand-navy">
              Your order
            </h3>

            <div className="mt-4 flex items-center justify-between">
              <span className="text-sm font-semibold uppercase tracking-wide text-brand-navy">Quantity</span>
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => setQuantity(quantity - 1)} disabled={quantity <= 1} aria-label="Decrease quantity" className={stepperBtn}>
                  −
                </button>
                <span aria-live="polite" className="w-6 text-center font-mkt-display text-lg font-bold text-brand-navy">
                  {quantity}
                </span>
                <button type="button" onClick={() => setQuantity(quantity + 1)} disabled={quantity >= 5} aria-label="Increase quantity" className={stepperBtn}>
                  +
                </button>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-1 border-t border-brand-navy/10 pt-4 text-sm text-brand-ink/90">
              <div className="flex justify-between">
                <span>{quantity} x {packTitle}</span>
            ...
            <span aria-live="polite" className="sr-only">
              {quantity} {packTitle}
              {quantity > 1 ? "s" : ""}
              {alphabetQty > 0 ? ` and ${alphabetQty} letter set${alphabetQty > 1 ? "s" : ""}` : ""}. Total {formatUsd(total)}.
            </span>
            ...
            <p className="mt-3 text-center text-xs text-brand-ink/70">{copy.buy.guarantee}</p>
            {error && (
