...
export function OrderFulfiller({ orderId, sessionId }: { orderId: string; sessionId: string }) {
  ...
  return (
    <div className="mt-6 rounded-lg border border-brand-navy-soft/40 bg-brand-cream-soft px-6 py-5">
      {status === "working" && (
        <p className="text-base text-brand-ink/80">Preparing your proof and sending it to your inbox…</p>
      )}
      {status === "done" && (
