...
## SYNTHESIS — fix-now blocker batch (all small)
- [ ] B-tech1: redirect/delete orphaned /checkout + /confirmation (no-payment order pages)
- [ ] B-trust1: make /classic non-public (fabricated reviews, FTC)
- [ ] B-ux1: proof-render dead-end — timeout in handleReview + don't hard-gate Add to cart on proof
- [ ] B-quick: /cart robots disallow; webhook payment_status!=unpaid gate; contact route returns non-200 on email failure

## Owner confirmations (cannot do in code)
- [ ] Confirm Railway env: DATABASE_URL, RESEND_API_KEY, PRODUCTION_EMAILS, STRIPE_WEBHOOK_SECRET
- [ ] Decide: firm "before the Fourth" promise — commit or soften?
- [ ] Confirm: are named "early tester" quotes (Melissa C./Jacob S./Nicole A.) real people?
